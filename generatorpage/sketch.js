...
function setup() {
  ...
  let b = random(4, 8);
  for (let i = 0; i < b; i++) {
    shapeM[i] = new shapeMid(random(width), random(height), random(40, (radiusSlider.value())));
  }
...
function draw() {
  ...
  for (let i = 0; i < shapeM.length; i++) {
    shapeM[i].radiusChange(radiusSlider.value());
    shapeM[i].move(); //bounce off the edges
    shapeM[i].show();
  }
...
class shapeMid {
  ...
  show() {
    ...
    beginShape();
    for (let a = 0; a < TWO_PI; a += 0.1) {
      let offset = map(noise(xoff, this.yoff), 0, 1, -50, 50);
